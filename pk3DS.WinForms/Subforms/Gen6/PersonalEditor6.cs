...
public partial class PersonalEditor6 : Form
{
    ...
    */
    ...
    private void B_CopyTM_Click(object sender, EventArgs e)
    {
        var sb = new StringBuilder(CLB_TMHM.Items.Count);
        for (int i = 0; i < CLB_TMHM.Items.Count; i++)
            sb.Append(CLB_TMHM.GetItemChecked(i) ? "1" : "0");
        Clipboard.SetText(sb.ToString());
    }

    private void B_PasteTM_Click(object sender, EventArgs e)
    {
        if (!Clipboard.ContainsText()) return;
        string text = Clipboard.GetText().Trim();
        if (text.Length != CLB_TMHM.Items.Count)
        {
            WinFormsUtil.Error("Clipboard string length does not match the TM/HM list length.");
            return;
        }
        for (int i = 0; i < text.Length; i++)
        {
            if (text[i] == '1') CLB_TMHM.SetItemChecked(i, true);
            else if (text[i] == '0') CLB_TMHM.SetItemChecked(i, false);
        }
    }
}